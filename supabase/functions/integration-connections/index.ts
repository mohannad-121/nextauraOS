...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body.operation === 'revoke' || body.operation === 'delete') { const now = new Date().toISOString(); const { error } = await admin.from('integration_connections').update({ status: 'revoked', revoked_at: now, updated_at: now }).eq('id', connectionId).eq('organization_id', organizationId); if (error) throw error; await audit(admin, organizationId, user.id, body.operation === 'delete' ? 'integration.connection.deleted' : 'integration.connection.revoked', connectionId); return json({ success: true }); }
    if (body.operation === 'test') { if (connection.status !== 'active') return json({ success: false, error: 'Only active connections can be tested.' }, 409); await admin.from('integration_connections').update({ last_verified_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq('id', connectionId).eq('organization_id', organizationId); await audit(admin, organizationId, user.id, 'integration.connection.tested', connectionId); return json({ success: true, status: 'metadata_verified' }); }
