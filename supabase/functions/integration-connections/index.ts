...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body.operation === 'startGoogleOAuth') {
      ...
      const state = randomOAuthState(); const expiresAt = new Date(Date.now() + 10 * 60_000).toISOString(); const { error } = await admin.from('integration_oauth_states').insert({ state_hash: await sha256(state), provider: 'google', organization_id: organizationId, user_id: user.id, connection_id: connectionId, expires_at: expiresAt }); if (error) throw error;
      const client = googleClient(); const authorizationUrl = new URL('https://accounts.google.com/o/oauth2/v2/auth'); authorizationUrl.searchParams.set('client_id', client.clientId); authorizationUrl.searchParams.set('redirect_uri', client.redirectUri); authorizationUrl.searchParams.set('response_type', 'code'); authorizationUrl.searchParams.set('scope', GOOGLE_FOUNDATION_SCOPES.join(' ')); authorizationUrl.searchParams.set('state', state); authorizationUrl.searchParams.set('access_type', 'offline'); authorizationUrl.searchParams.set('prompt', 'consent');
