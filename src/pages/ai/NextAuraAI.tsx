...
} from '../../services/nextAuraAIService';
...
export const NextAuraAI: React.FC = () => {
  const { currentOrg, user, activeServices, aiEntryContext, navigate } = useApp();
  ...
  const completeRequest = async (conversationId: string, requestMessages: Array<{ role: 'user' | 'assistant'; content: string }>) => {
    ...
    try {
      const response = await nextAuraAIService.ask(
        currentOrg.id,
        aiEntryContext.app,
        aiEntryContext.subView,
        requestMessages,
        { name: currentOrg.name, role: user.role, activeServices },
      );
    ...
    <aside aria-label="Request context" className="h-full border-s border-slate-200 bg-[#FAFAF8] p-5">
      ...
      <dl className="mt-6 space-y-5 text-xs">
        ...
        <div>
          ...
          <dd className="mt-1.5 capitalize text-slate-700">{aiEntryContext.app} / {aiEntryContext.subView}</dd>
        </div>
        <div>
      ...
      <div className="mt-7 border-t border-slate-200 pt-5 text-[11px] leading-5 text-slate-500">
        Answers are generated by NextAura's local rules and knowledge base. Relevant workspace data remains tenant-scoped and permission-aware.
      </div>
    </aside>
  );
          ...
          <header className="flex items-center justify-between gap-3 border-b border-slate-200 px-4 py-3.5 sm:px-6">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="truncate text-xs text-slate-500">Your intelligent NextAura assistant</p>
              </div>
            </div>
            <div className="flex items-center gap-1">
              ...
              <button type="button" onClick={clearConversation} disabled={!activeConversation?.messages.length} className="flex h-10 w-10 items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 disabled:cursor-not-allowed disabled:opacity-35" aria-label="Clear conversation"><Eraser className="h-[18px] w-[18px]" /></button>
                      ...
                      {message.role === 'user' ? (
                        ...
                        <div className="flex items-start gap-3.5">
                          ...
                          <div className="min-w-0 flex-1">
                            <MessageResponse className="text-[15px] leading-7 text-slate-800">{message.content}</MessageResponse>
                              ...
                              <div className="mt-4 flex flex-wrap gap-2" aria-label="Suggested follow-up questions">
                                {message.suggestions.map((suggestion) => (
                                  <button
                                    ...
                                    disabled={isLoading}
                                    className="rounded-full border border-blue-200 bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-800 transition-colors hover:border-blue-300 hover:bg-blue-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2 disabled:opacity-50"
                                  >
                                    {suggestion.label}
                                  </button>
                                ))}
          ...
          <div className="border-t border-slate-200 bg-white px-4 py-4 sm:px-6">
            {error && (
              <div className="mx-auto mb-3 flex max-w-3xl items-start justify-between gap-4 border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-900" role="alert">
                <div><p className="font-semibold">Request not completed</p><p className="mt-1 leading-5">{error.message}</p></div>
                {retryPrompt && <button type="button" onClick={retryLastRequest} className="inline-flex shrink-0 items-center gap-1.5 font-semibold hover:text-amber-700"><RefreshCw className="h-3.5 w-3.5" />Retry</button>}
              </div>
            )}
