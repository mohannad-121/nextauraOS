    ...
    <header className="sticky top-0 z-20 flex h-16 items-center border-b border-slate-200/70 bg-white/90 px-3 backdrop-blur-xl dark:border-slate-800 dark:bg-[#222825]/90 sm:px-5 lg:px-7">
      <div className="flex min-w-0 flex-1 items-center gap-3">
        ...
        <div className="relative hidden lg:block" ref={companyMenuRef}>
          <button type="button" onClick={() => setCompanyMenuOpen((open) => !open)} aria-expanded={companyMenuOpen} className="flex max-w-52 items-center gap-2 rounded-xl px-2.5 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800">
            <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-blue-50 text-blue-700 dark:bg-blue-950/50 dark:text-blue-300"><Building2 className="h-3.5 w-3.5" /></span>
            <span className="truncate">{currentOrg.name}</span><ChevronDown className="h-3.5 w-3.5 text-slate-400" />
          </button>
          {companyMenuOpen && <div className="absolute start-0 top-full z-50 mt-2 w-72 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl dark:border-slate-700 dark:bg-slate-900">
