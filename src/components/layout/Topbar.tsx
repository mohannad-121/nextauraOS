...
export const Topbar: React.FC = () => {
  ...
  useEffect(() => {
    const closeMenu = (event: MouseEvent) => {
      ...
      if (companyMenuRef.current && !companyMenuRef.current.contains(event.target as Node)) setCompanyMenuOpen(false);
    };
  ...
  useEffect(() => {
    const closeOnEscape = (event: KeyboardEvent) => { if (event.key === 'Escape') setCompanyMenuOpen(false); };
    document.addEventListener('keydown', closeOnEscape);
    return () => document.removeEventListener('keydown', closeOnEscape);
  }, []);

  const selectCompany = async (organizationId: string) => {
    if (organizationId === currentOrg.id || switchingOrgId) return;
    setSwitchingOrgId(organizationId); setCompanyError('');
    try { await switchOrg(organizationId); setCompanyMenuOpen(false); }
    catch (error: any) { setCompanyError(error?.message || 'Could not switch company.'); }
    finally { setSwitchingOrgId(null); }
  };
  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center border-b border-slate-200/70 bg-white/90 px-3 backdrop-blur-xl dark:border-slate-800 dark:bg-[#222825]/90 sm:px-5 lg:px-7">
      ...
      <div className="flex flex-1 items-center justify-end gap-1 sm:gap-1.5">
        <div className="relative hidden shrink-0 md:block" ref={companyMenuRef}>
          <button
            type="button"
            onClick={() => setCompanyMenuOpen((open) => !open)}
            aria-label={`Switch company, currently ${currentOrg.name}`}
            aria-expanded={companyMenuOpen}
            className="flex h-10 max-w-56 items-center gap-2 rounded-xl border border-slate-200 bg-white px-2.5 text-xs font-semibold text-slate-700 shadow-sm transition-colors hover:border-slate-300 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:hover:border-slate-600 dark:hover:bg-slate-700"
          >
            <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-blue-50 text-blue-700 dark:bg-blue-950/50 dark:text-blue-300"><Building2 className="h-3.5 w-3.5" aria-hidden="true" /></span>
            <span className="truncate">{currentOrg.name}</span>
            <ChevronDown className="h-3.5 w-3.5 shrink-0 text-slate-400" aria-hidden="true" />
          </button>
          {companyMenuOpen && <div className="absolute end-0 top-full z-[60] mt-2 w-72 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl dark:border-slate-700 dark:bg-slate-900">
            <p className="px-2.5 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-slate-400">Companies</p>
            {organizations.map((org) => { const unavailable = org.lifecycleStatus && org.lifecycleStatus !== 'active'; const active = currentOrg.id === org.id; return <button key={org.id} type="button" disabled={Boolean(unavailable || switchingOrgId)} onClick={() => void selectCompany(org.id)} className={`flex w-full items-center justify-between rounded-xl px-2.5 py-2.5 text-start text-xs ${unavailable ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800'} ${active ? 'bg-blue-50 text-blue-700 dark:bg-blue-950/40 dark:text-blue-300' : 'text-slate-700 dark:text-slate-200'}`}><span className="flex min-w-0 items-center gap-2"><Building2 className="h-4 w-4 shrink-0" /><span className="truncate">{org.name}</span>{org.billingRootOrganizationId === null && <span className="text-[10px] text-slate-400">Main company</span>}</span>{switchingOrgId === org.id ? <Loader2 className="h-4 w-4 animate-spin" /> : unavailable ? <span className="flex items-center gap-1 text-[10px]"><Lock className="h-3 w-3" />{org.lifecycleStatus === 'archived' ? 'Archived' : 'Requires Custom'}</span> : active ? <Check className="h-4 w-4" /> : null}</button>; })}
            {companyError && <p className="px-2.5 pt-2 text-[11px] text-rose-600">{companyError}</p>}
            {companyCreationAccess !== 'none' && <div className="mt-1 border-t border-slate-100 pt-1 dark:border-slate-800"><button type="button" disabled={companyCreationAccess === 'loading'} onClick={() => { if (companyCreationAccess === 'upgrade') navigate('pricing'); else setCompanyCreationOpen(true); setCompanyMenuOpen(false); }} className="flex w-full items-center gap-2 rounded-xl px-2.5 py-2.5 text-xs font-semibold text-blue-700 hover:bg-blue-50 disabled:cursor-wait disabled:opacity-60 dark:text-blue-300 dark:hover:bg-blue-950/30"><Plus className="h-4 w-4" />{companyCreationAccess === 'loading' ? 'Checking company access…' : companyCreationAccess === 'upgrade' ? 'Upgrade for more companies' : companyCreationAccess === 'error' ? 'Company access unavailable' : 'Add company'}</button></div>}
          </div>}
        </div>
        {contextualAction && (
          <button
